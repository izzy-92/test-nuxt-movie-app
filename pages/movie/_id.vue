...
export default {
  ...
  // 실제 서버사이드 렌더링이 발생하기 전에 동작하는 Nuxt 제공 옵션
  async asyncData({ store, params }) { //context 내에서 구조분해한 store
  ...
  head() {
    return {
      // 해당 head의 meta는 nuxt.config.js에서 지정한 head 옵션과 하위 meta 옵션을 지칭한다.
      // meta 옵션을 통해 새롭게 덮어써야하는 내용을 명시 (hid가 동일해야 정확하게 덮어쓸수있음)
      meta: [
        // 영화 상세정보 페이지에서 바로 meta 정보를 정리해주기 위해 붙여넣음.
        ...
        // process.env.CLIENT_URL/movie/영화id
